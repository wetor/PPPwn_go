...
func (pppoe *PPPoE) SetTags(tags []Tag) {
	pppoe.tags = tags
}
...
func (pppoe *PPPoE) BuildPADO() *Pkt {
	padi := new(Pkt)
	padi.Code = CodePADO
	padi.SessionID = 0
	padi.Tags = pppoe.tags
...
func (pppoe *PPPoE) Send(pkt *Pkt, etype uint16, addr net.HardwareAddr) (n int, err error) {
	if atomic.LoadUint32(pppoe.state) != pppoeStateOpen {
		return 0, fmt.Errorf("pppoe is not open")
	}
	pktbytes, err := pkt.Serialize()
	...
	_, err = pppoe.conn.WritePktTo(pktbytes, etype, addr)
	if err != nil {
		return 0, fmt.Errorf("failed to send pppoe pkt,%w", err)
	}
...
func (pppoe *PPPoE) ReadFrom(buf []byte) (int, net.HardwareAddr, error) {
	if atomic.LoadUint32(pppoe.state) != pppoeStateOpen {
